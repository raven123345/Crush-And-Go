Shader "Raven/Toon_Smoothstep"
{
    ...
    SubShader
    {
        ...
        {
        //diffuse light
            fixed _lightDir = dot(lightDir, s.Normal);
            fixed _lightClip = smoothstep(0.0,_Step, _lightDir * atten);

        //ambient
            fixed4 ambient = lerp(_AmbientColor,(1,1,1,1),_lightClip);
        ...
        //combinning
            fixed4 color;
            color.rgb = s.Albedo * _LightColor0 * ambient.rgb + specClip; //s.Albedo * ambient * _LightColor0;
            color.a= s.Alpha;
